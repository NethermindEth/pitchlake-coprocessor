...
use simulate_price_fixed::simulate_price;
use simulate_price_fixed_methods::SIMULATE_PRICE_FIXED_GUEST_ID;
use tokio::main;
...
async fn main() -> Result<(), String> {
    ...
    let xxxx = de_seasonalised_detrended_log_base_fee
        .iter()
        .map(|x| FixedPoint::from_num(*x))
        .collect::<Vec<_>>();
    let de_seasonalised_detrended_log_base_fee_fixed =
        DVector::from_iterator(xxxx.len(), xxxx.into_iter());

    let receipt = simulate_price(&de_seasonalised_detrended_log_base_fee_fixed);
